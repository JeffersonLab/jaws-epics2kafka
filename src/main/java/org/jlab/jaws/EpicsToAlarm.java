...
/**
 * Transforms records from epics2kafka to JAWS format. Special hints are provided to ensure AVRO
 * messages honor enum and union types.
 *
 * @param <R> The Record to transform
 */
public abstract class EpicsToAlarm<R extends ConnectRecord<R>> implements Transformation<R> {
  public static final String OVERVIEW_DOC = "Transform epics2kafka messages to JAWS format";

  public static final SevrEnum[] sevrByOrder = new SevrEnum[4];

  // We removed NO_ALARM since it cannot exist downstream as it's replaced with either a tombstone
  // or NoActivation
  // marker.  However, incoming records have it, so we can no longer simply assign SevrEnum.values()
  // as is done
  // with StateEnum.
  static {
  ...
  static final Schema updatedValueSchema =
      inputData.toConnectSchema(AlarmActivationUnion.getClassSchema());
  ...
  /**
   * This transform can either use tombstones to indicate no activation or use the NoActivation
   * marker entity.
   *
   * <p>The USE_NO_ACTIVATION environment variable is consulted and unless the value == "false", the
   * default is to use the NoActivation marker.
   *
   * <p>The advantage of the marker is that it can be aggressively compacted and maintains the
   * number of messages in the topic at roughly equal to the number of registered alarms IF
   * aggressive compaction is configured. The downside is the number of messages can never be less
   * than registered alarms. Best for workloads with spontaneous nuisance alarms.
   *
   * <p>The advantage of the tombstone is that in well-behaved alarm systems (few nuisance alarms),
   * the number of messages in the alarm-activations topic can be quite low. The disadvantage is
   * that tombstones cannot be compacted and only expire based on delete.retention.ms, which means
   * if you have a lot of toggling alarms (nuisance alarms), then they are only cleaned up based on
   * delete.retention.ms, which may need to be quite long since that effectively caps the amount of
   * time that can elapse before a materialized view must check for updates so that it can
   * incrementally catch-up to the latest state, otherwise the view must be entirely rebuilt from
   * scratch (re-query entire topic) to avoid missing deleted tombstones (compaction keeps the
   * latest state of a topic EXCEPT tombstones).
   *
  ...
  /**
   * Apply transformation to the {@code record} and return another record object (which may be
   * {@code record} itself) or {@code null}, corresponding to a map or filter operation
   * respectively.
   *
  ...
  public static class Value<R extends ConnectRecord<R>> extends EpicsToAlarm<R> {
    ...
    protected Struct doUpdate(Struct original, Schema updatedSchema) {
      ...
      if (!alarm && USE_NO_ACTIVATION) {
        union.put(
            "org.jlab.jaws.entity.NoActivation",
